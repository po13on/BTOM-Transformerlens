...
| 「找 A-→V 的头」 | Phase 1 → **goal-directed** loop → accumulate matches + continue upward → `visualize_model_heads` once → report |
| 「完整归因树 / 自动发现电路」 | Phase 1 → **open-ended** Stage0 `A-->V`×{q,k,v} → frontier expand → role ledger → viz once → report |
| 「假设数据只有 A-->V，重新归因」 | Seed `Answer`/`AnswerSpan` only → discover readable roles → expand → compare to labeled run |
| 「点一下可视化」 | Insert/run `visualize_model_heads(...)` once; do **not** click the widget for the user |
...
4. **Every Phase 2 round = three new code cells** (never overwrite prior rounds; never merge cluster+expand):
...
5. OOM / missing weights / CUDA issues → stop unless the fix is obvious and safe.
6. Scope to the **newest** user ask (load-only ≠ full attribution).
7. Modes:
...
8. Stage 0 exits: **multi-pathway union** — each selected `(L,H)` gets `attn_q`+`attn_k`+`attn_v`. Robustness = same pathway across exits, **not** swap-q/k-then-intersect.
9. `print_tree` often **collapses** mixed patterns on a depth; truth is the **role ledger** / on-tree `(pattern, type)` list. Always report Section 3 for complete trees.
...
2. **Stage 1+:** new cluster/score/expand cells; cover **both** A−-side and V-side when candidates exist.
...
- Pattern Score screens; Positive Bound is for click-review in `visualize_model_heads` — never conflate.
- Never pass raw `index_map['A-']` as `pos_ids`.
- End review: **one** `visualize_model_heads` cell; no legacy `colored_tokens_multi` for-loop.
- Goal-directed: first match ≠ stop. Open-ended: first `A-->V` expand ≠ done.

## Do not

- Phase 2 outside the notebook; merge cluster+expand; overwrite prior Round cells.
