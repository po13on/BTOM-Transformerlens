...
from attribute import attribute_attn_weights, get_examples, get_ranges, get_pos_ids_by_span
...
def get_head_matching_scores(result, attn_patterns, model, layer, head, last_k=2):
    ...
    if isinstance(model, transformer_lens.HookedTransformer):
        token_ids = model.to_tokens(result.prompt, prepend_bos=False)[0]
        raw_tokens = model.tokenizer.convert_ids_to_tokens(token_ids.tolist())
    else:
        raw_tokens = model.tokenizer.tokenize(result.prompt)
    ...
    for attn_pattern in _attn_patterns:
        if is_mlp:
            ...
            continue
        attn_labels, ranges_q = attn_pattern2labels(get_examples(result), attn_pattern, tokens)
...
def eval_head_lens(result, model, layer, head, strict=False, reduce='mean'):
    ...
    output = get_head_output(model, result, layer, head, pos_ids) \
        if head < (model.cfg.n_heads if isinstance(model, transformer_lens.HookedTransformer) else model.config.num_attention_heads) else \
...
def _normalize_show_attn_pos_ids(r, pos_ids=None, last_k=1):
    """Normalize query positions for ``show_attn``.

    Returns:
        pos_ids: 1D LongTensor of query indices
        use_answer_labels: True when defaulting to answer/A- positions
    """
    if pos_ids is None:
        return torch.LongTensor(r.answer_indices[-last_k:]) - 1, True
    if isinstance(pos_ids, torch.Tensor):
        return pos_ids.detach().cpu().long().reshape(-1), False
    if isinstance(pos_ids, int):
        return torch.LongTensor([pos_ids]), False
    return torch.as_tensor(list(pos_ids), dtype=torch.long).reshape(-1), False


def _format_tokens_at(tokens, positions):
    """Join token strings at the given indices for attention-row labels."""
    parts = []
    for p in positions:
        p = int(p)
        if 0 <= p < len(tokens):
            tok = tokens[p]
            parts.append(tok if tok.strip() != '' else repr(tok))
        else:
            parts.append(f'@{p}')
    return ''.join(parts) if len(parts) <= 1 else '|'.join(parts)


def _default_attn_start(r, last_k=1):
    """Token window start: index_map (grid jsonl) → Hi-ToM input_ranges → 0."""
    try:
        return r.index_map[0]["start"]
    except Exception:
        pass
    try:
        return r.puzzle["train"][-last_k].input_ranges.prefix["grid"][0] - 5
    except Exception:
        pass
    return 0
...
def show_attn(r, model, layer, head, downstreams=None, last_k=1, start=None, stop=None,
              pos_ids=None, target_pos_ids=None):
    """Visualize attention from query ``pos_ids``.

    Args:
        pos_ids: Query positions (attention rows). Default: A- / answer indices - 1.
        target_pos_ids: Symbolic attended positions used for the attention-row label
            (e.g. V for pattern ``A-->V``). When set, labels show those tokens instead
            of answer strings / query indices.
    """
    ...
    pos_ids, use_answer_labels = _normalize_show_attn_pos_ids(r, pos_ids=pos_ids, last_k=last_k)
    if target_pos_ids is not None:
        target_pos_ids, _ = _normalize_show_attn_pos_ids(r, pos_ids=target_pos_ids, last_k=last_k)
        use_answer_labels = False
    ...
    if use_answer_labels:
        ...
        # Label = attended / pattern-src token(s) (e.g. V for A-->V), not query index.
        if target_pos_ids is None:
            target_pos_ids = pos_ids
        target_label = _format_tokens_at(tokens, target_pos_ids.tolist())
        labels = ['act'] + join_lists(
            [['q->', target_label] + (['aa'] if downstreams is not None else []) for _ in pos_ids.tolist()]
        )
    start = start if start is not None else _default_attn_start(r, last_k)
    stop = stop if stop is not None else len(tokens)
    print(
        f'start={start}, stop={stop}, pos_ids={pos_ids.tolist()}'
        + (f', target_pos_ids={target_pos_ids.tolist()}' if target_pos_ids is not None else '')
    )
    return tokens[start:stop], aw[0, :, start:stop].T, labels


def show_attn_grid(r, model, layer, head, downstreams=None, last_k=1, start=None, stop=None,
                   pos_ids=None, target_pos_ids=None):
    """Grid-puzzle attention: same computation as ``show_attn``, Test-span window.

    Default ``start`` is ``r.index_map[0]["start"]`` (jsonl ``spans`` Test 段).
    Default ``pos_ids`` remains ``answer_indices - 1``. Does not read ``input_ranges``.
    """
    if start is None:
        start = r.index_map[0]["start"]
    return show_attn(
        r, model, layer, head,
        downstreams=downstreams, last_k=last_k,
        start=start, stop=stop, pos_ids=pos_ids, target_pos_ids=target_pos_ids,
    )
...
def compute_cosine_matrix(
    ...
    # Flatten each head to a 1D vector and stack. Per-sample lists (variable-length
    # datasets) are concatenated; all heads share the same sample set, so lengths
    # match. Zero-padding is exact for cosine (zeros add nothing to dot/norm), so
    # any residual length mismatch is padded, never truncated.
    vectors = []
    for k in head_keys:
        v = data_dict[k]
        if isinstance(v, (list, tuple)):
            v = torch.cat([t.float().flatten() for t in v])
        else:
            v = v.float().flatten()
        vectors.append(v)
    max_len = max(v.numel() for v in vectors)
    vectors = [torch.nn.functional.pad(v, (0, max_len - v.numel())) for v in vectors]
...
def compute_js_matrix(aw_dict: Dict[Tuple[int, int], torch.Tensor]) -> Tuple[np.ndarray, List[Tuple[int, int]]]:
    ...
    Args:
        aw_dict: Dict mapping (layer, head) -> attention weights, either a dense
                 (batch, query_len, kv_len) tensor or a per-sample list of tensors
                 (variable-length datasets, see attribute.get_attn_attrs_on_dataset).
                 Lists are compared sample-by-sample on their own support — no
                 padding, no truncation — and every real query row contributes
                 equally to the mean.
    ...
    def _js_rows(ta: torch.Tensor, tb: torch.Tensor) -> Tuple[float, int]:
        """Summed JS over all (batch, query) rows of one paired sample."""
        p, q = ta.float(), tb.float()
        if p.shape != q.shape:
            # Defensive union-pad: never drop real mass to force equal length.
            kv = max(p.shape[-1], q.shape[-1])
            qr = max(p.shape[-2], q.shape[-2]) if p.ndim > 1 else 1
            p = torch.nn.functional.pad(p, (0, kv - p.shape[-1], 0, qr - p.shape[-2]))
            q = torch.nn.functional.pad(q, (0, kv - q.shape[-1], 0, qr - q.shape[-2]))
        # Smooth to avoid log(0), then renormalize each row
        p = p + eps
        q = q + eps
        p = p / p.sum(dim=-1, keepdim=True)
        q = q / q.sum(dim=-1, keepdim=True)
        m = (p + q) / 2
        js_rows = 0.5 * ((p * torch.log(p / m)).sum(dim=-1) + (q * torch.log(q / m)).sum(dim=-1))
        return float(js_rows.sum().item()), int(js_rows.numel())
    ...
    for i, k1 in enumerate(head_keys):
        for j, k2 in enumerate(head_keys):
            if i >= j:
                continue
            total, rows = 0.0, 0
            for ta, tb in zip(_as_list(aw_dict[k1]), _as_list(aw_dict[k2])):
                s, c = _js_rows(ta, tb)
                total += s
                rows += c
            js_val = total / max(rows, 1)
...
def cluster_heads(
    ...
    if model_config is not None:
        if isinstance(model_config, transformer_lens.HookedTransformerConfig):
            num_heads = model_config.n_heads
            num_kv_heads = model_config.n_key_value_heads
    ...
    if show_plot or save_path:
        ...
        # Heatmap
        pos = dist_matrix[dist_matrix > 0]
        vmax = min(vmax_default, float(np.percentile(pos, 95))) if pos.size else vmax_default
    ...
    for idx, cluster_id in enumerate(sorted(groups.keys())):
        ...
        for head in group_heads:
            ...
            if (l, h) in aw_dict:
                v = aw_dict[(l, h)]
                if isinstance(v, (list, tuple)):
                    # Per-sample list (variable-length): union-pad with zeros, then
                    # average. Display-only approximation — short samples contribute
                    # zeros beyond their own length.
                    kv = max(t.shape[-1] for t in v)
                    qr = max(t.shape[-2] for t in v) if v[0].ndim > 1 else 1
                    v = torch.stack([
                        torch.nn.functional.pad(t, (0, kv - t.shape[-1], 0, qr - t.shape[-2]))
                        for t in v
                    ])
                patterns.append(v.mean(dim=0))
        if not patterns:
            continue
...
def collect_tree_attn_heads(root):
    ...
    return set(collect_tree_attn_nodes(root).keys())


def collect_tree_attn_nodes(root):
    """Map (layer, head) -> attribution ``Node`` objects on the tree (skip lm_head).

    The same (layer, head) may appear multiple times with different ``type`` /
    ``attn_pattern`` (e.g. attn_q vs attn_k). Order follows tree traversal.
    """
    nodes_by_lh = {}
    for tnode in iter_attribution_tnodes(root):
        ...
        for n in getattr(data, 'nodes', None) or []:
            ...
            if hasattr(n, 'is_lm_head') and n.is_lm_head():
                continue
            key = (int(n.layer), int(n.head))
            nodes_by_lh.setdefault(key, []).append(n)
    return nodes_by_lh


def resolve_tree_node_query_pos_ids(node, sample):
    """Recompute attribution query positions for ``sample`` (mutates ``node.pos_ids``).

    ``show_attn`` / ``colored_tokens_multi`` need the **query** index used during
    attribution (``node.pos_ids``), not ``src_pos_ids`` (K/V source span).
    """
    node.set_pos_ids(sample)
    pos = getattr(node, 'pos_ids', None)
    if pos is None:
        return None
    if isinstance(pos, torch.Tensor):
        return pos.detach().cpu().long().reshape(-1)
    return torch.as_tensor(pos, dtype=torch.long).reshape(-1)


def resolve_pattern_target_pos_ids(node, sample):
    """Positions of the pattern's attended side (right of ``->``), e.g. V for ``A-->V``.

    Used as the attention-row label in ``show_attn``, independent of Q/K/V pathway.
    """
    pattern = getattr(node, 'attn_pattern', None)
    if not pattern or '->' not in pattern:
        return None
    _, src = pattern.split('->', 1)
    src = src.strip()
    try:
        if src in ('A-', 'A'):
            # Same convention as lm_head / show_attn default: last prompt token.
            return torch.LongTensor(sample.answer_indices) - 1
        return get_pos_ids_by_span(sample, src).detach().cpu().long().reshape(-1)
    except Exception:
        return None
...
def visualize_model_heads(
    ...
    """Interactive L×H head map: click a circle to render attention on the right.
    ...
    - For **on-tree** heads, query ``pos_ids`` are recomputed from the attribution
      ``Node`` via ``set_pos_ids(sample)`` (same query index used during attribution),
      not hard-coded to A-. Off-tree heads still default to A-.

    Args:
        root: Attribution tree root (pptree node with ``.data.nodes``).
        model: HookedTransformer or HF model used by ``attn_fn``.
        results: List of result objects (same as ``_results``).
        sample: Optional initial sample; defaults to ``results[0]``.
        downstreams: Passed to ``attn_fn`` for off-tree heads / fallback; defaults to
            ``root.data.nodes``. On-tree clicks prefer that head node's own downstreams.
        start: Token window start for ``attn_fn``; defaults to
        ...
        attn_fn: Attention visualizer, ``show_attn`` (Hi-ToM, default) or
            ``show_attn_grid`` (move_to_match). Called with the same kwargs
            previously passed to ``show_attn``.
    ...
    tree_nodes = collect_tree_attn_nodes(root)
    on_tree = set(tree_nodes.keys())
    ...
    for l in range(n_layers):
        for h in range(n_heads):
            ...
            if marked:
                n0 = tree_nodes[(l, h)][0]
                pat = getattr(n0, 'attn_pattern', None) or '?'
                ntype = getattr(n0, 'type', '?')
                meta = f' · {ntype} · {pat}'
            texts.append(f'L{l} H{h}' + meta)
            custom.append([l, h])
    ...
    def _resolve_tree_context(l, h, s):
        """For on-tree heads, use attribution query/target pos + that node's downstreams."""
        nodes = tree_nodes.get((l, h))
        if not nodes:
            return None, None, _resolve_downstreams_default(), None
        node = nodes[0]
        try:
            q_pos = resolve_tree_node_query_pos_ids(node, s)
        except Exception as e:
            return None, None, (node.get_downstreams() or _resolve_downstreams_default()), (
                f'{type(e).__name__}: {e}'
            )
        # Pattern right-hand side (e.g. V in A-->V) = attended target for the aw label.
        t_pos = resolve_pattern_target_pos_ids(node, s)
        ds = node.get_downstreams() or _resolve_downstreams_default()
        meta = {
            'type': getattr(node, 'type', None),
            'pattern': getattr(node, 'attn_pattern', None),
            'span': getattr(node, 'span', None),
            'src_span': getattr(node, 'src_span', None),
            'pos_ids': q_pos.tolist() if q_pos is not None else None,
            'target_pos_ids': t_pos.tolist() if t_pos is not None else None,
            'n_aliases': len(nodes),
        }
        return q_pos, t_pos, ds, meta

    def _resolve_downstreams_default():
        if downstreams is not None:
            return downstreams
    ...
    def render_head(l, h):
        state['sel'] = (l, h)
        s = state['sample']
        q_pos, t_pos, ds, meta = _resolve_tree_context(l, h, s)
        if meta is None:
            status.value = (
                f'<b>Selected</b> layer={l}, head={h}'
                f' &nbsp;|&nbsp; query pos: default A-'
            )
        elif isinstance(meta, str):
            status.value = (
                f'<b>Selected</b> layer={l}, head={h}'
                f' &nbsp;|&nbsp; failed to resolve tree pos_ids: {meta}'
                f' &nbsp;|&nbsp; falling back to default A-'
            )
            q_pos, t_pos = None, None
        else:
            status.value = (
                f'<b>Selected</b> layer={l}, head={h}'
                f' &nbsp;|&nbsp; {meta["type"]} / {meta["pattern"]}'
                f' &nbsp;|&nbsp; query={meta["pos_ids"]}'
                f' &nbsp;|&nbsp; target={meta["target_pos_ids"]}'
                + (f' &nbsp;|&nbsp; (+{meta["n_aliases"]-1} aliases)' if meta["n_aliases"] > 1 else '')
            )
        with attn_out:
            clear_output(wait=True)
            display(colored_tokens_multi(*attn_fn(
                s, model, int(l), int(h),
                downstreams=ds,
                start=_resolve_start(s),
                pos_ids=q_pos,
                target_pos_ids=t_pos,
            )))
